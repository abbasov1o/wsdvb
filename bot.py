...
def mention_all(update: Update, context: CallbackContext) -> None:
    ...
    if any(filter(lambda x: x in update.message.text, config["keywords"])):
        ...
        if str(chat.id) not in chatid_usersid:
            try:
                ...
                update.message.reply_text(
                    "İstifadəçilər qeyd olunmur! Xahiş edirəm botu admin olaraq tanıtın və 'İstifadəçiləri link vasitəsi ilə dəvət edin' və 'Mesajları silin'!"
                )
                return
        ...
        try:
            ...
            update.message.reply_text(
                f"Başqalarına məlumat verildi {len(users)} Fərdi！" + tag_text.rstrip(),
                parse_mode=ParseMode.MARKDOWN,
            )
